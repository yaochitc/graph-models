...
class NodeLabelPartition(val labels: LongFloatVector) extends Serializable {
  def init(model: SupervisedGNNPSModel): Unit =
    model.setLabels(labels)
}

object NodeLabelPartition {
  def apply(iterator: Iterator[(Long, Float)], dim: Long): NodeLabelPartition = {
    val labels = VFactory.sparseLongKeyFloatVector(dim)
    while (iterator.hasNext) {
      val entry = iterator.next()
      labels.set(entry._1, entry._2)
    }
    new NodeLabelPartition(labels)
  }
}